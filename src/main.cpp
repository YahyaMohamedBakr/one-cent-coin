...
#if defined(NDEBUG)
# error "Onecentcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x4a565c4c897d51698de0eecb612e882cd5f4da08b92c672c88d0006fe6f70936");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Onecentcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Onecentcoin Signed Message:\n";
...
{
    // Onecentcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Onecentcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Onecentcoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Onecentcoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Onecentcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Onecentcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4a565c4c897d51698de0eecb612e882cd5f4da08b92c672c88d0006fe6f70936");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is onecentcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1514651260;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 6719968;
        ...
        assert(block.hashMerkleRoot == uint256("0x9aa6bd1248f66faa23cafb1744db0328b1860c303caf62c45a479d59a06c2ad3"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Onecentcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OnecentcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OnecentcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static OnecentcoinMiner(CWallet *pwallet)
{
    printf("OnecentcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("onecentcoin-miner");
    ...
    try { loop {
        ...
        printf("Running OnecentcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("OnecentcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&OnecentcoinMiner, pwallet));
}
